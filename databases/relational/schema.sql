...
--
-- # TASK 6 EXTENSION:
-- This file integrates the Task 6 Lost Items and Penalties tables.
--
...
-- =========================================================================
-- TASK 6 EXTENSION:
-- RATIONALE: We use an ENUM for lost_item_status to enforce a strict state machine.
-- This prevents invalid statuses (e.g., 'lost', 'missing') and ensures data integrity
-- when tracking an item from 'reported' to 'claimed' or 'police'.
CREATE TYPE lost_item_status AS ENUM ('reported', 'found', 'claimed', 'police', 'donated', 'destroyed', 'love_umbrella');

-- RATIONALE: The lost_items table includes an 'is_high_value' boolean.
-- Why? High-value items (wallets, phones) require different handling protocols (e.g., police handover)
-- compared to low-value items (umbrellas).
...
-- =========================================================================
-- TASK 6 EXTENSION:
-- RATIONALE: Penalty status is tracked via ENUM to manage the lifecycle of a fine.
-- A penalty starts as 'unpaid', can be 'appealed', and ends as 'paid'.
CREATE TYPE penalty_status AS ENUM ('unpaid', 'paid', 'appealed');

-- RATIONALE: Penalties are linked to a user_id rather than a specific booking_id.
-- Why? Because violations like 'fare evasion' occur precisely when a user DOES NOT have a booking.
-- Tracking 'due_date' and 'paid_at' separately enables financial auditing for overdue fines.
