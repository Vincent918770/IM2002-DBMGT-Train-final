        ...
        with conn.cursor(cursor_factory=psycopg2.extras.RealDictCursor) as cur:
            ...
            if service_type == "normal":
                ...
                if hours_until_departure >= 48:
                    refund_pct, admin_fee, policy_note = 100, 0.00, "RF001_W1: ??8h ??100% refund, no fee"
                elif hours_until_departure >= 24:
                    refund_pct, admin_fee, policy_note = 75, 0.50, "RF001_W2: 24??8h ??75% refund, $0.50 fee"
                elif hours_until_departure >= 2:
                    refund_pct, admin_fee, policy_note = 50, 0.50, "RF001_W3: 2??4h ??50% refund, $0.50 fee"
                else:
                    refund_pct, admin_fee, policy_note = 0, 0.00, "RF001_W4: <2h or past departure ??no refund"
            else:
                # RF002 windows (express ??stricter)
                if hours_until_departure >= 48:
                    refund_pct, admin_fee, policy_note = 100, 1.00, "RF002_W1: ??8h ??100% refund, $1.00 fee"
                elif hours_until_departure >= 24:
                    refund_pct, admin_fee, policy_note = 50, 1.00, "RF002_W2: 24??8h ??50% refund, $1.00 fee"
                else:
