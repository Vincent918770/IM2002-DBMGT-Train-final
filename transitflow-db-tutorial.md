...
## 繁體中文總結 (Chinese Summary)

這份教學文件以 TransitFlow 專案為基礎，詳細介紹了關聯式資料庫、圖形資料庫及向量資料庫的設計實務。以下為各章節重點摘要：

**第一部分：基礎觀念**
*   **多資料庫架構 (Polyglot Persistence)**：專案依賴三種引擎各司其職。PostgreSQL 處理結構化交易資料；pgvector 處理政策文件的語意搜尋；Neo4j 處理複雜的交通網路與路線規劃。
*   **命名與資料型別**：推薦統一使用複數、snake_case 命名。金額一律使用 `DECIMAL` 以避免浮點數誤差；時間則使用 `TIMESTAMPTZ` 以確保時區正確。
*   **正規化與反正規化**：介紹第一到第三正規化 (1NF-3NF)。在某些特定場景 (如不會輕易更動的火車座位配置)，為了查詢效能，可以刻意將結構「反正規化 (Denormalized)」，將資料攤平在一張表中。

**第二部分：Schema 設計實務**
*   **主鍵 (Primary Key) 選擇**：教學如何依據情況選擇 `SERIAL` (內部表)、`UUID` (隱藏數量)、`Natural Key` (穩定編碼，如車站代碼) 或應用程式產生的字串。
*   **外鍵與刪除機制**：分析 `CASCADE`、`RESTRICT` 與 `SET NULL` 的差異。對於商業資料，強烈建議使用「軟刪除 (Soft Delete)」搭配 `RESTRICT`，而非實體刪除。
*   **索引 (Indexes)**：講解 B-Tree、JSONB 適用的 GIN 以及向量搜尋的 HNSW。並說明不該盲目建索引，因為會拖慢寫入效能。
*   **JSONB 與多型關聯**：介紹如何將陣列資料存入 JSONB；以及如何利用「字串前綴 (String-Prefix)」來實作多型關聯 (Polymorphic Associations)，讓一個外鍵可以對應到兩張不同的表。

**第三到第七部分：進階技巧**
*   **資訊安全**：強烈警告不能明文儲存密碼。應使用 `bcrypt` 或現代標準的 `argon2id` 加密；並使用參數化查詢 (`%s`) 來防止 SQL Injection 攻擊。
*   **交易與檢視表 (Transactions & Views)**：利用 `BEGIN/COMMIT/ROLLBACK` 確保資料一致性；透過 `SELECT ... FOR UPDATE` 避免重複訂票 (Race Condition)。
*   **Neo4j 與 Cypher**：展示圖形資料庫如何輕鬆解決 SQL 很難處理的 N 節點路徑搜尋與最短路徑規劃 (Dijkstra) 問題。
*   **開發實務**：種子腳本應具有「冪等性 (Idempotent)」，利用 `ON CONFLICT DO NOTHING`，使其能重複執行而不報錯。
